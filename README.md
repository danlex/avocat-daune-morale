# avocat-daune-morale.ro

Single-page Jekyll site for **Cabinet de Avocat VĂRZARU ION**, recreating the content of
`avocat-daune-morale.ro` with a modern template (shared with `avocat-despagubiri-victime.ro`
and `avocat-asigurari-rca.ro`, inspired by `varzaru.ro`). Deployed on GitHub Pages at the custom
domain **avocat-daune-morale.ro**.
...
│   ├── daune-morale.html  echipa.html  intrebari.html  testimoniale.html  contact.html
│   └── structured-data.html
├── assets/css/style.scss   # Theme (compiled to style.css by Jekyll)
├── content/                # (reserved) source content, excluded from build
├── _config.yml             # Site + contact settings
├── CNAME                   # Custom domain: avocat-daune-morale.ro
...
3. The `CNAME` file already sets the custom domain to `avocat-daune-morale.ro`. In
...
- **Apex `avocat-daune-morale.ro`** → four `A` records:
